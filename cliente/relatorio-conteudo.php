...
$sql = "SELECT * FROM tbl_usuario inner join tbl_conteudo_usr on tbl_usuario.id_usr = tbl_conteudo_usr.id_usr WHERE tbl_usuario.id_usr = $id AND tbl_conteudo_usr.id_usr = $id ORDER BY data_cont DESC";
     ...
     <div class="container" style="margin-bottom:200px;">
      ...
      <div class="row">
          <div class="col-md-12" style="text-align:center;margin:0 0 50px 0;">
            ...
            <strong>Total de registros enviados por <?php echo $_GET['nome']; ?>: <?php echo $total ?></strong>
          </div>
      </div>  
          ...
          <div class="col-md-4" style="text-align:left;">
            <?php
              if ($user['status_cont'] == 'N') {
                echo '<a href="publicar-conteudo.php?status=S&idcont='.$user['id_cont'].'" class="btn btn-success" role="button">PUBLICAR</a>';
              } else {
                echo '<a href="publicar-conteudo.php?status=N&idcont='.$user['id_cont'].'" class="btn btn-warning" role="button">REMOVER</a>';
              }
            ?>  
          </div>    
      </div>  
